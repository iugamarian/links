...
static volatile unsigned char interrupt_detected = 0;
static volatile unsigned char finetuning_wanted = 0;
static volatile unsigned char fine_tuning_value_minus = 0;
static volatile unsigned char fine_tuning_value_plus = 0;
...
unsigned char fine_tuning_value_for_loop = 0;
...
void led_update_finetuning() {

// How it works:

// do not display normally all except year area with fine tuning
...
// Year display will now show fine_tuning_value_minus with the two points off and
// fine_tuning_value_plus with the two points on
// Potentiometer selection for year decrease and year increase will modify
// fine_tuning_value_minus and fine_tuning_value_plus not year value
// Potentiometer selection for other than increase year and decrease year are
// allowed to modify their respective values but will exit very fast the fine tuning
// mode so only one value will be changed, can fix immediately with the opposite button
...
ledday = 88;
ledmonth = 88;
// ledyear will be defined more down here for this case of led_update_finetuning
...
//      
  ...
  yearover100detected = 0; // let it still have this original name to be able to use same display function for the year area
  ledyear = fine_tuning_value_minus;
  if(fine_tuning_value_plus > 0)
	{
		yearover100detected = 1;
		ledyear = fine_tuning_value_plus;
	}
...
void potentiometerdecreasefinetuning() {
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	if(fine_tuning_value_plus == 0) {
		fine_tuning_value_minus++;
	}
	if(fine_tuning_value_minus > 99) {
		fine_tuning_value_minus = 99;
	}
	if(fine_tuning_value_minus == 0) {
		fine_tuning_value_plus--;
	}
}

void potentiometerincreasefinetuning() {
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	if(fine_tuning_value_minus == 0) {
		fine_tuning_value_plus++;
	}
	if(fine_tuning_value_plus > 99) {
		fine_tuning_value_plus = 99;
	}
	if(fine_tuning_value_plus == 0) {
		fine_tuning_value_minus--;
	}
...
void do_month_finetuning() {
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	if(fine_tuning_value_minus > 0) {
		fine_tuning_value_for_loop = fine_tuning_value_minus;
		while(fine_tuning_value_for_loop > 0)
		{
			if(seconds == 0) {
				seconds = 59;
			...
			fine_tuning_value_for_loop--;
		}
	}

	if(fine_tuning_value_plus > 0) {
		fine_tuning_value_for_loop = fine_tuning_value_plus;
		while(fine_tuning_value_for_loop > 0)
		{
			if(seconds == 59) {
				seconds = 0;
...
int main(void) {
	...
	// Needed also because interrupt tuning is only 1 minute precise every month
	fine_tuning_value_minus = 0; // minus seconds in 1 month for time correction, 0 to 99, fine_tuning_value_plus is 0 when this being larger than zero
	fine_tuning_value_plus = 0; // plus seconds in 1 month for time correction, 0 to 99, fine_tuning_value_minus is 0 when this being larger than zero
	// to enter fine tuning selection decrease year from 2019 to 2199, increasing the year from 2199 to 2019 will not open fine tuning mode
	// to modify fine tuning value use decrease year and increase year
	// to exit fine tuning selection press any other than year decrease or increase button
	// with interrupt register tuning we can fix minutes every month, but can still be up to 30 seconds off for 1 month
	// with fine tuning we can fix seconds every month, but can still be up to 6 seconds off for 1 year, this is acceptable I think.
	while(1)
    		{
...
//        250 - 255 = 5 steps = year increase - easy a to set at maximum position
		...
		if((pot_value < 5) && (finetuning_wanted == 0))
			{
			pot_status = 17;
			potentiometerdecreaseyears();
			}
			
		if((pot_value < 5) && (finetuning_wanted == 1))
			{
			pot_status = 17;
			potentiometerdecreasefinetuning();
			}
...
//        250 - 255 = 5 steps = year increase - easy a to set at maximum position
		...
		if((pot_value > 249) && (finetuning_wanted == 0))
			{
			pot_status = 27;
			potentiometerincreaseyears();
			}
		if((pot_value > 249) && (finetuning_wanted == 1))
			{
			pot_status = 27;
			potentiometerincreasefinetuning();
			}
			
///////////  ACT ON INTERRUPT DETECTED AT THE PASS OF A MINUTE

		if((interrupt_detected == 1) && (seconds == 0) && (finetuning_wanted == 0))
			{
...
// Potentiometer is not at constant mode, display changed status:
		...
		if((pot_status > 0) && (finetuning_wanted == 0))
			{
			led_update_fields();
...
// Potentiometer is at constant mode, display changed status one more time if led_update_needed is 1, then set led_update_needed to 0:
		...
		if(finetuning_wanted == 1)
			{
			led_update_finetuning(); // display fine tuning "screen" on every loop pass:
			// FINE
			// TUNE
			//  22
			//  ..
			led_update_needed = 0; // avoid displaying led_update_fields();
			finetuning_wanted = 0;
			if(pot_status == 0)
				{
				finetuning_wanted = 1;
